...
def train_multitask(args):
    ...
    sst_train_data = torch.utils.data.Subset(sst_train_data, indices=np.arange(8500))
    ...
    sts_train_data = torch.utils.data.Subset(sts_train_data, indices=np.arange(6000))
    sts_dev_data = SentencePairDataset(sts_dev_data, args, isRegression=True)
    ###############


    sst_train_dataloader = DataLoader(sst_train_data, shuffle=True, batch_size=50,
                                      collate_fn=sst_train_data.collate_fn)
    assert len(sst_train_dataloader) == 8500
    sst_dev_dataloader = DataLoader(sst_dev_data, shuffle=False, batch_size=args.batch_size,
                                    collate_fn=sst_dev_data.collate_fn)
    ##### Added #####
    para_train_dataloader = DataLoader(para_train_data, shuffle=True, batch_size=50,
                                      collate_fn=para_train_data.collate_fn) 
    para_dev_dataloader = DataLoader(para_dev_data, shuffle=False, batch_size=args.batch_size,
                                    collate_fn=para_dev_data.collate_fn)
    sts_train_dataloader = DataLoader(sts_train_data, shuffle=True, batch_size=50,
