...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # S = S.masked_fill(attention_mask == 1, -1e5)
    S += attention_mask
    S = F.softmax(S, dim=-1)
    # S = self.dropout(S)
    V_prime = torch.matmul(S, value)
    V_prime = torch.reshape(V_prime.transpose(1, 2), (S.size(dim=0), S.size(dim=-1), self.num_attention_heads * self.attention_head_size))
    return V_prime
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    ### TODO
    mhal_output = self.self_attention.forward(hidden_states, attention_mask)
    addnorm_output = self.add_norm(hidden_states, mhal_output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    ff_output = self.interm_af(self.interm_dense(addnorm_output))
    addnorm_output_2 = self.add_norm(addnorm_output, ff_output, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    embeddings = torch.add(inputs_embeds, pos_embeds)
    embeddings = torch.add(embeddings, tk_type_embeds)
