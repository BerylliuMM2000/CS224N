...
### Try using the PCGrad package for gradient surgery
...
def train_multitask(args):
    ...
    ######## Use gradient surgery from the PCGrad package #######
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = 0
        num_batches = 0
        # losses = [[] for i in range(2000)]
        ################# Try iterating through datasets and get one set of loss a time ###############
        ...
        for i in range(10000):
            ...
            batch3 = next(it3)
            optimizer.zero_grad()
            # Get loss from sentiment classifier
            b_ids1, b_mask1, b_labels1 = (batch1['token_ids'],
            ...
            # Get loss from paraphrase detecter
            (b_ids21, b_mask21,
            b_ids22, b_mask22,
            b_labels2, b_sent_ids2) = (batch2['token_ids_1'], batch2['attention_mask_1'],
                        batch2['token_ids_2'], batch2['attention_mask_2'],
                        batch2['labels'], batch2['sent_ids'])

            b_ids21 = b_ids21.to(device)
            b_mask21 = b_mask21.to(device)
            b_ids22 = b_ids22.to(device)
            b_mask22 = b_mask22.to(device)
            ...
            # Get loss from similarity
            (b_ids31, b_mask31,
            b_ids32, b_mask32,
            b_labels3, b_sent_ids3) = (batch3['token_ids_1'], batch3['attention_mask_1'],
                        batch3['token_ids_2'], batch3['attention_mask_2'],
                        batch3['labels'], batch3['sent_ids'])

            b_ids31 = b_ids31.to(device)
            b_mask31 = b_mask31.to(device)
            b_ids32 = b_ids32.to(device)
            b_mask32 = b_mask32.to(device)
        ...
        #     loss = F.cross_entropy(logits, b_labels.view(-1), reduction='sum') / 50
        #     losses[0].extend(loss)
        ...
        #     loss2 = F.binary_cross_entropy_with_logits(logits2, b_labels2.float().unsqueeze(1), reduction='sum') / 50
        #     losses[1].extend(loss2)
        ...
        #     mse = F.mse_loss
        #     # Normalize [0, 5] to [-1, 1] to align with cosine similarity
        #     loss3 = mse(logits3.flatten(), (b_labels3.float()/2.5-1)) / 50
        #     losses[2].extend(loss3)
        #     # loss3.backward()
        #     # optimizer.step()
        #     # num_batches += 1
        #     # train_loss += loss3.item()
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'grad-surg-{args.option}-{args.epochs}-{args.lr}-multitask.pt' # save path
